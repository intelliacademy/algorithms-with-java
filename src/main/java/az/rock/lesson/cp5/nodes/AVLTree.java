...
public class AVLTree<T extends Comparable<T>> implements Tree<T> {
    ...
    private void reBalance(AbstractNode<T> node) {
        while(node != null && !node.isEmpty()) {
            ...
            node = node.getParent();
            if (node != null)
                System.out.println("Result is balanced? : " + node.getBalance());
        }
    }
    ...
    public AbstractNode<T> reBalancing(AbstractNode<T> node) {
        if (node.isLeftHeavy()){
            if (node.leftChild.isRightHeavy()){
                node.setLeftChild(this.rotateLeft(node.leftChild));
            }
            node = this.rotateRight(node);
        }  if (node.isRightHeavy()){
            if (node.rightChild.isLeftHeavy()){
                node.setRightChild(this.rotateRight(node.rightChild));
            }
...
//
//        if(tempLeftChild.getParent() != null && !tempLeftChild.getParent().isEmpty() && tempLeftChild.getParent().getLeftChild().equals(node)){
//            tempLeftChild.getParent().setLeftChild(tempLeftChild);
//        } else if (tempLeftChild.getParent() != null && !tempLeftChild.getParent().isEmpty()  && tempLeftChild.getParent().getRightChild().equals(node)){
